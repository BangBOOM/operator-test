...
def testBatchPrefillWithRaggedKVCacheWrapper(args):
    ...
    if kv_dtype not in [torch.bfloat16, torch.float8_e4m3fn, torch.float8_e5m2]:
        print(f"[ERROR] Unsupported kv_dtype: {args.kv_dtype}")
        return res
    if q_dtype in [torch.float8_e4m3fn, torch.float8_e5m2] and args.out_dtype is None:
        print(
            "[ERROR] --out_dtype must be set to bfloat16 or float16 when --q_dtype is FP8."
        )
        return res
...
def testBatchMLAPagedAttentionWrapper(args):
    ...
    if "cutlass" in backends:
        ...
        if s_qo != 1:
            print(
                "[INFO] Cutlass MLA benchmark backend only supports s_qo == 1. Skipping."
            )
            remove_cutlass = True
    ...
    total_qo_len = batch_size * s_qo
    q_nope = torch.rand(
        total_qo_len, num_qo_heads, head_dim_ckv, dtype=q_init_dtype, device="cuda"
    )
    q_pe = torch.zeros(
        total_qo_len, num_qo_heads, head_dim_kpe, dtype=q_init_dtype, device="cuda"
    )
    ...
    qo_indptr = torch.arange(0, total_qo_len + 1, s_qo, device=device).int()
    kv_indptr = (
        torch.cat(
            [
                torch.tensor([0], device=device),
    ...
    def run_backend_wrapper(
        ...
        if backend in ["fa2", "fa3"]:
            ...
            return flashinfer.mla.trtllm_batch_decode_with_kv_cache_mla(
                query=q.reshape(batch_size, s_qo, num_qo_heads, head_dim_ckv + head_dim_kpe),
                ...
                bmm1_scale=sm_scale,
                bmm2_scale=1.0,
            ).reshape(total_qo_len, num_qo_heads, head_dim_ckv)
        elif backend == "cute-dsl":
            return flashinfer.mla.trtllm_batch_decode_with_kv_cache_mla(
                query=q.reshape(batch_size, s_qo, num_qo_heads, head_dim_ckv + head_dim_kpe),
                kv_cache=kv_cache.unsqueeze(1),
    ...
    for backend in backends:
        if len(backend_times[backend]) > 0:
            ...
            o_mem_bytes = total_qo_len * num_qo_heads * head_dim_ckv * o_elem_size
