...
def test_mla(
    ...
    def profile_label(name):
        mode = "nps" if non_persistent_mode else "ps"
        var = "varlen" if varlen else "fixed"
        lse = "lse" if return_lse else "nolse"
        return (
            f"{name}_c{ctx_lens}_b{batch_size}_h{nhead}_kvh{nhead_kv}"
            f"_q{decode_qlen}_blk{page_size}_ms{max_split_per_batch}"
            f"_{_dtype_name(dtype)}_{_dtype_name(kvtype)}_{paged_layout}_{mode}_{var}_{lse}"
        )

    def profile_mla_decode(name, *decode_args, **decode_kwargs):
        _run_torch_profile(
            profile_label(name),
            torch_profile_dir,
    ...
    def test_absorb_decode_bf16_fp8():
        ...
        profile_mla_decode("absorb_bf16_fp8", *decode_args, **decode_kwargs)
    ...
    def test_absorb_decode_bf16():
        ...
        profile_mla_decode("absorb_bf16", *decode_args, **decode_kwargs)
    ...
    def test_absorb_decode_fp8():
        ...
        profile_mla_decode("absorb_fp8", *decode_args, **decode_kwargs)
    ...
    def test_absorb_decode_3buffer():
        ...
        profile_mla_decode("absorb_3buffer", *decode_args, **decode_kwargs)
...
parser.add_argument(
    ...
    default=None,
    metavar="DIR",
    help="""enable torch.profiler for the MLA decode call and write chrome trace json to DIR.
    e.g.: --torch-profile ./mla_traces""",
)
parser.add_argument(
    "--torch-profile-warmup",
    type=int,
    default=3,
    help="""torch profiler warmup iterations. Default: 3.
